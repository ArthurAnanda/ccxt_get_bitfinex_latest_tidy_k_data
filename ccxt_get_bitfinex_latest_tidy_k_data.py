...
'''
author: Arthur Luo
version: Python 3.6.4 |Anaconda custom (64-bit)
'''
...
def ccxt_get_bitfinex_latest_k_data(symbol='BTC/USDT', timeframe='5m', limit=1000, timeout=1000):
    ...
    end_minute = (now_datetime.minute // time_frame) * time_frame
    ...
    time_end = timetemp - datetime.timedelta(minutes=time_frame)
    timeendstr = time_end.strftime('%Y-%m-%d %H:%M:%S')
    timestart = timetemp - datetime.timedelta(minutes=time_frame * limit)
